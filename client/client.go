...
func runCommand(ctx context.Context, client pb.CallServiceClient, params string) error {
	fmt.Printf("RPC Call with params (%q)\n", params)
	r, err := client.Call(ctx, &pb.Params{Params: params})
	if err != nil {
...
func RunClient(serverAddress string, params string) error {
	...
	defer conn.Close()
	c := pb.NewCallServiceClient(conn)
